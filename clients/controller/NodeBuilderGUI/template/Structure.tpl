<div class="table-responsive">
    <table class="table table-bordered table-striped table-hover">
        ...
        <tbody>
            ...
            <tr>
            	<td><span class="glyphicon glyphicon-{if($NodeField.inform == -1)}ban{else}ok{/if}-circle"></span></td>
            	<td><a href="{$FieldAction}EditField/{$NodeField.name}/">{$NodeField.label}</a></td>
                <td>{$NodeField.name}</td>
                <td>{$NodeField.type}</td>
                <td><span class="glyphicon glyphicon-{if($NodeField.require == -1)}ban{else}ok{/if}-circle"></span></td>
